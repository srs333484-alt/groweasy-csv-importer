...
exports.uploadCSV = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Failed to process CSV: ' + error.message });
  }
};
...
exports.processCSV = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Failed to process records: ' + error.message });
  }
};
